...
public class goban9 : MonoBehaviour
{

    private const int BOARD_SIZE = 9;

    private Stone9[,] stones = new Stone9[BOARD_SIZE,BOARD_SIZE];
    ...
    public Vector3 zonaAleatoriaBlancas = new Vector3(-1.5f, 0.8f, 5f); // Rango del área aleatoria en XYZ
    public Vector3 zonaAleatoriaNegras = new Vector3(10f, 0.8f, 5.2f); // Rango del área aleatoria en XYZ
    ...
    private Stone9 playStone(string color, int x, int y)
    {        
        Stone9 stone = Instantiate(go_stone).GetComponent<Stone9>();
        ...
        stones[x,y] = stone;
        Point9 p = points[x,y].GetComponent<Point9>();
        p.x=stone.x;
        p.y=stone.y;
        p.stone9=stone;
    ...
    public void setConnections(Point9 point)
    {
        Stone9 stone = point.gameObject.transform.GetChild(0).GetComponent<Stone9>();
        int x = stone.x;
        int y = stone.y;
        string team = point.gameObject.tag;

        List<Point9> neighbors = new List<Point9>();

        if (y+1 >= 0 && y+1 <= 8)
        {Point9 north = points[(int)x,(int)y+1].GetComponent<Point9>();neighbors.Add(north);}
        if (x+1 >= 0 && x+1 <= 8)
        {Point9 east = points[(int)x+1,(int)y].GetComponent<Point9>();neighbors.Add(east);}
        if (y-1 >= 0 && y-1 <= 8)
        {Point9 south = points[(int)x,(int)y-1].GetComponent<Point9>();neighbors.Add(south);}
        if (x-1 >= 0 && x-1 <= 8)
        {Point9 west = points[(int)x-1,(int)y].GetComponent<Point9>();neighbors.Add(west);}

        foreach (Point9 n in neighbors)
        {
            GameObject go = n.gameObject;
            if (go.tag == team)
            {
                if (!point.GetComponent<Point9>().connections.Contains(go.GetComponent<Point9>()))
                    point.GetComponent<Point9>().connections.Add(go.GetComponent<Point9>());
                if (!go.GetComponent<Point9>().connections.Contains(point.GetComponent<Point9>()))
                    go.GetComponent<Point9>().connections.Add(point.GetComponent<Point9>());
            }
        }
    }

    public HashSet<Point9> getGroup(Point9 point)
    {
        HashSet<Point9> visited = new HashSet<Point9>();
        
        if (point.connections.Contains(point))
                return visited;
            
        var queue = new Queue<Point9>();
        queue.Enqueue(point);
        ...
        while (queue.Count > 0)
        {
            ...
            foreach(Point9 neighbor in vertex.connections)
    ...
    public void sendGroup(Point9 point)
    {
        foreach (Point9 member in point.group)
        {
            foreach (Point9 p in point.group)
    ...
    public void getNeighbors(Point9 point)
    {
        if (point.y+1 < 9)
        {
            Point9 north = points[(int)point.x,(int)point.y+1].GetComponent<Point9>();
            point.neighbors.Add(north);
        }
        if (point.x+1 < 9)
        {
            Point9 east = points[(int)point.x+1,(int)point.y].GetComponent<Point9>();
            point.neighbors.Add(east);
        }
        if (point.y-1 > -1)
        {
            Point9 south = points[(int)point.x,(int)point.y-1].GetComponent<Point9>();
            point.neighbors.Add(south);
        }
        if (point.x-1 > -1)
        {
            Point9 west = points[(int)point.x-1,(int)point.y].GetComponent<Point9>();
            point.neighbors.Add(west);
        }
    }    

    public void getReach(Point9 point)
    {
        foreach (Point9 member in point.group)
            foreach( Point9 neighbor in member.neighbors)
                if (!point.reached.Contains(neighbor))
                    point.reached.Add(neighbor);
    }

    public void sendReach(Point9 point)
    {
        foreach (Point9 member in point.group)
            foreach (Point9 p in point.reached)
                if (!member.reached.Contains(p))
                    member.reached.Add(p);
    }

    public bool validGroup(Point9 point)
    {
        bool libertyFound = false;
        foreach (Point9 reach in point.reached)
    ...
    //public void killGroup(Point9 point)
    //{
    //    foreach (Point9 member in point.group)
    //    {
    ...
    public void connectStones(Point9 point)
    {
    ...
    public void checkNeighbors(Point9 point)
    {
        foreach (Point9 neighbor in point.neighbors)
